        ...
        <section style={{ maxWidth: "1100px", margin: "0 auto", padding: isMobile ? "10px" : "20px", color: "#1c1917", fontFamily: "sans-serif" }}>
            ...
            {showValuationModal && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
                    <div style={{ background: 'white', padding: '30px', borderRadius: '24px', maxWidth: '500px', width: '100%', position: 'relative', boxShadow: '0 20px 40px rgba(0,0,0,0.2)' }}>
                        <button onClick={() => setShowValuationModal(false)} style={{ position: 'absolute', right: '20px', top: '20px', border: 'none', background: 'none', fontSize: '24px', cursor: 'pointer', color: '#1c1917' }}>✕</button>
                        <h3 style={{ color: '#A67C00', fontSize: '22px', marginBottom: '15px' }}>Equity Calculation</h3>
                        <p style={{ fontSize: '15px', lineHeight: '1.6', color: '#444', marginBottom: '20px' }}>
                            Based on current Essex/London market trends, added habitable space is valued at a average baseline of <strong>£1,500 per sqm</strong>.
                        </p>
                        <div style={{ background: '#fafaf9', padding: '20px', borderRadius: '16px', border: '1px solid #e7e5e4' }}>
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                ...
                                <strong style={{ color: '#16a34a' }}>+ £{(size * 2000).toLocaleString()}</strong>
                            </div>
                        </div>
                        <p style={{ fontSize: '12px', color: '#78716c', marginTop: '20px', lineHeight: '1.4' }}>
                            <em>Disclaimer: Crafman Design & Build provide construction estimates. We are not a regulated valuation company. Market values fluctuate based on location and finish quality.</em>
                        </p>
                    </div>
                </div>
            )}
                    ...
                    <div ref={resultsRef}>
                        <div style={{ background: "#1c1917", color: "#fff", borderRadius: "24px", padding: isMobile ? "24px" : "35px", height: "100%", position: 'relative' }}>
                            ...
                            <div style={{ marginBottom: '25px', background: 'rgba(255,255,255,0.04)', padding: '18px', borderRadius: '18px', border: '1px dashed rgba(166, 124, 0, 0.4)' }}>
                                ...
                                <button onClick={() => setShowValuationModal(true)} style={{ background: 'none', border: 'none', color: '#a8a29e', fontSize: '12px', textDecoration: 'underline', padding: '5px 0', cursor: 'pointer' }}>
                            ...
                            <div style={{ marginTop: "30px", paddingTop: "24px", borderTop: "1px solid rgba(255,255,255,0.1)" }}>
                                ...
                                <IncludedItem label="External Rendering / Brickwork" />
                                <IncludedItem label="Electrical & Plumbing" />
                                <IncludedItem label="Plastering & Drylining" />
                                ...
                                <div style={{ display: "flex", justifyContent: "space-between", margin: "14px 0", fontSize: "15px", color: "#d6d3d1" }}>
                                    <span>{getDoorLabel()} (Install):</span>
                                    <span style={{ filter: showResult ? "none" : "blur(8px)" }}>£{totals.doorCost.toLocaleString()}</span>
                                </div>

                                {lead.addSkylights && (
                                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "14px", fontSize: "15px", color: "#d6d3d1" }}>
                                        <span>Skylights Installation:</span>
                                        <span style={{ filter: showResult ? "none" : "blur(8px)" }}>£{totals.skylightCost.toLocaleString()}</span>
                                    </div>
                                )}
                                ...
                                {lead.addKitchen && (
                                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "14px", fontSize: "15px", color: "#d6d3d1" }}>
                                        ...
                                        <span style={{ filter: showResult ? "none" : "blur(8px)" }}>£{baseRates.kitchenInstall.toLocaleString()}</span>
                                    </div>
                                )}
                                ...
                                {lead.addBathroom && (
                                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "14px", fontSize: "15px", color: "#d6d3d1" }}>
                                        ...
                                        <span style={{ filter: showResult ? "none" : "blur(8px)" }}>£{baseRates.bathroomInstall.toLocaleString()}</span>
                                    </div>
                                )}
                                ...
                                {lead.addFlooring && (
                                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "14px", fontSize: "15px", color: "#d6d3d1" }}>
                                        ...
                                        <span style={{ filter: showResult ? "none" : "blur(8px)" }}>£{totals.flooringCost.toLocaleString()}</span>
                                    </div>
                                )}

                                {showResult && (
                                    <div style={{ marginTop: "30px" }}>
                                        <div style={{ fontSize: "11px", color: "#a8a29e", marginBottom: "20px", fontStyle: "italic" }}>
                                            * Baseline turnkey estimate. Confirmed upon site survey.
                                        </div>
                                        <div style={{ background: "rgba(166, 124, 0, 0.15)", padding: "18px", borderRadius: "14px", fontSize: "13px", lineHeight: "1.6", borderLeft: "4px solid #A67C00", marginBottom: "25px" }}>
                                            <strong>Pro Tip:</strong> We often find design optimizations during our free sessions that can <strong>save you £3,000–£5,000</strong>.
                                        </div>
                                        <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                                            <a href="https://calendar.app.google/NBBKg6BiESkgfsJ5A" target="_blank" rel="noopener noreferrer" style={{ background: "#A67C00", color: "#fff", padding: "18px", borderRadius: "12px", textDecoration: "none", fontWeight: "800", textAlign: "center", fontSize: '15px' }}>📅 Book Free Concept Meeting</a>
                                            <a href="https://calendar.app.google/khdhBvuq446KCp1V9" target="_blank" rel="noopener noreferrer" style={{ background: "transparent", border: "2px solid #fff", color: "#fff", padding: "18px", borderRadius: "12px", textDecoration: "none", fontWeight: "800", textAlign: "center", fontSize: '15px' }}>🏢 Meet at Rainham Office</a>
                                        </div>
                                    </div>
                                )}
                            </div>
